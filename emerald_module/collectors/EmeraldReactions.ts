...
export class EmeraldReactionCollector extends EventEmitter {
	...
	listener: (message: Message, emoji: recievedEmoji, reactor: Member) => Promise<boolean>;

	deleted: (message: Message, emoji: recievedEmoji, userID: string) => Promise<void>;
	...
	constructor(options: ICollectorOptions) {
		...
		this.listener = (message, emoji, reactor) => this.checkPreConditions(message, emoji, reactor);

		this.deleted = (message, emoji, userID) => this.wasDeleted({ message, emoji, userID });
	...
	private async checkPreConditions(message: Message, emoji: recievedEmoji, reactor: Member) {
	 ...
	 */
	private async wasDeleted({ message, emoji, userID }: emeraldDisposedReaction) {
		if (
			this.collected.find(
				(item) => item.emoji.id === emoji.id && item.message.id === message.id && userID === item.reactor.id
			) !== undefined
		) {
			this.collected = this.collected.filter(
				(item) => !(emoji.id === item.emoji.id && message === item.message && userID === item.reactor.id)
			);
